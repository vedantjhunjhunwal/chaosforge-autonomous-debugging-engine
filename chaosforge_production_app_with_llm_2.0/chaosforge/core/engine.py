...
from chaosforge.llm.client import LLMClient
...
class ChaosForgeEngine:
    ...
    def execute_run(self, run_id: str, contract: TargetContract, runs: int = 100, concurrency: int = 8) -> dict:
        ...
        final = {"id": run_id, "target_name": contract.name, "status": "running", "artifact_dir": str(output_dir), "llm_enabled": self.llm.enabled, "llm_provider": self.llm.provider}
        ...
        try:
            ...
            for round_id in range(1, 4):
                batch = generate_payloads(round_id, contract=contract, scan=scan, llm=self.llm)
            ...
            if not crash:
                ...
                write_pr_artifact(output_dir, final, llm=self.llm)
            ...
            patch = generate_patch(mapped.get("file") or (contract.source_files[0] if contract.source_files else ""), result.crash_type, int(mapped.get("line") or 1), output_dir, crash_result=result.to_dict(), payload=payload, llm=self.llm)
            ...
            final.update({
                ...
                "llm_used": bool(patch.get("llm_used")) or any(p.get("generated_by") == "llm" for p in payloads),
